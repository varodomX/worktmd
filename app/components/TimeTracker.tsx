...
import { Upload, Download, Edit2, Save, X, AlertCircle, FileText, Copy } from 'lucide-react';
...
export default function TimeTracker() {
  ...
  const getEditingRowIndex = () =>
    editingId ? getCurrentData().findIndex((entry) => entry.id === editingId) : -1;

  const duplicatePreviousRow = () => {
    const currentData = getCurrentData();
    const currentIndex = getEditingRowIndex();
    const previousEntry = currentData[currentIndex - 1];
    const currentEntry = currentData[currentIndex];

    if (!previousEntry || !currentEntry) return;

    const keepCurrentDate = Boolean(currentEntry.isMissing || editValues.isMissing);

    setEditValues({
      ...editValues,
      name: previousEntry.name,
      date: keepCurrentDate
        ? (editValues.date || currentEntry.date || currentEntry.missingDateLabel || '')
        : previousEntry.date,
      startTime: previousEntry.startTime,
      endDate: keepCurrentDate
        ? (editValues.endDate || currentEntry.endDate || previousEntry.endDate)
        : previousEntry.endDate,
      endTime: previousEntry.endTime,
      hoursWorked: calculateHours(previousEntry.startTime, previousEntry.endTime),
      location: previousEntry.location,
      latCheckin: previousEntry.latCheckin,
      lonCheckin: previousEntry.lonCheckin,
      latCheckout: previousEntry.latCheckout,
      lonCheckout: previousEntry.lonCheckout,
      workGroup: previousEntry.workGroup,
              ...
              <table className="w-full border-collapse font-bold">
                <thead>
                  <tr className="bg-green-600 text-white">
                    ...
                    <th className="sticky right-0 z-20 border border-gray-400 px-3 py-2 text-center text-sm font-bold min-w-20 text-green-900 bg-green-600">แก้ไข</th>
                  </tr>
                </thead>
                <tbody>
                  {data.map((entry, idx) => (
